                ...
                <tbody>
                    @forelse ($stock_check_requests as $key => $each)
                        <tr>
                            ...
                            <td scope="col">{{ $each->updated_at->format('d-m-Y, h:i A') ?? 'N/A' }}</td>


                            <td>
                                <a class="" href="{{ route('tenant_stock_check_show', $each->id) }}"
                                    data-toggle="tooltip" title="View details of this Stock check">
                                    Show |
                                </a>

                                <!-- Delete button with SweetAlert -->
                                <a href="#" type="button" data-toggle="tooltip" title="Delete this Stock Check Request"
                                    onclick="deleteRequest({{ $each->id }})">
                                    Delete
                                </a>
                                <!-- Delete form (hidden, used for sending DELETE request) -->
                                <form id="deleteForm{{ $each->id }}" method="POST"
                                    action="{{ route('tenant_stock_check_destroy', $each->id) }}" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
...
@section('script')
    ...
    <script>
        function deleteRequest(stockCheckRequestID) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('deleteForm' + stockCheckRequestID).submit();
                }
            });
        }
    </script>
