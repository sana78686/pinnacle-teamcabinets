                            ...
                            <div class="border col-xl-7 col-lg-7 col-md-12 col-sm-12">
                                ...
                                <div class="box box-primary" style="overflow-x: auto;">
                                    <div class="form-inline ">
                                        ...
                                        <button type="button" class="m-2 text-white btn btn-info"
                                                id="add_job_btn">
                                                + Add Job</button>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateTotals() {
                ...
                $(".custom_all_product_total").text(`$${totalprice.toFixed(2)}`);
                $(".custom_all_product_total").text(`$${totalWeight.toFixed(2)lbs}`);
            }
            $("#add_job_btn").click(function() {
                let jobName = $("#job_name").val().trim();
                if (jobName === "") {
                ...
                let roomRow = `
              <tr class ="shaw_room table-warning" >
                <td>
                    <label>Room <span class="astirisk font-danger">*</span> </label>
                </td>
                <td colspan = "5">
                    <input type ="text" name="roomlabel[]" placeholder = "Enter Room Name"  required/>
                </td>
                <td>
                    <button class="btn btn-outline-danger remove_room ">X</button>
                </td>
            ...
            $(document).on("dbclick", "[id^=product_label_]", function() {
                if (!lastRoomRow) {
                    alert("Please add aroombefore select a Product.")
                    return;
                }
            });
            let productRow = $(this).closest("tr");
            ...
            let productSku = productRow.find("td:nth-child(2)").text().trim();
            let productDescription = productRow.find("td:nth-child(3)").text().trim();
            let productWeight = parseFloat(productRow.find("td:nth-child(5)").text().replace().replace("$",
                "")) || 0;
            let existingProduct = $(`#room_table_body tr[data-sku ="${productSku}"]`);
            if (existingProduct.length > 0) {
                let qtyField = existingProduct.find(".product_quantity");
                let newQty = parseInt(newQtyField.text()) + 1;
                qtyField.text(newQty);
                let totalPriceField = existingField.find(".product_title_price");
                totalPriceField.text(`$${(newQty * productPrice).toFixed(2)}`);
                let totalWeightField = existingProduct.find(".product_weight");
                totalWeightField.text(`${(newQty * productWeight).toFixed(2)} lbs`);
            } else {
            ...
            <tr data-sku = "${productSku}">
                ...
                <input type="check-box" class = "single_check"><br>
                <input  type="check-box" class = "double_check">
                <td>${productLabel}</td>
                <td>${productDescription}</td>
                <td class="product_weight">$${productWeight.toFixed(2)}lbs</td>
                  <td>$${productPrice.toFixed(2)}</td>
        ...
        //    check only one check box is checked ata time
        $(docuent).on("change",".single_check",function(){
            if($(this).is(":checked")){
                $(this).closest("tr").find(".sigle_check").prop("checked",false)
            }
        });
        $(document).on("change","double_check",function(){
           if($(this).is(":checked")){
