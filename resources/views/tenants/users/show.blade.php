        ...
        <div class="row">
            ...
            <div class="p-2 col-xs-6 col-sm-6 col-md-6 col-lg-4">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($user->catalogVisibilities as $catalogVisibility)
                            <tr>
                                <th>{{ $catalogVisibility->productCatalog->name ?? 'N/A' }}</th>
                                <td>
                                    @php
                                        $doorFactors = $user
                                            ->doorFactors()
                                            ->where('catalog_id', $catalogVisibility->catalog_id)
                                            ->get();
                                    @endphp
                                    @if ($doorFactors->isNotEmpty())
                                        <ul class="mb-0 list-unstyled">
                                            @foreach ($doorFactors as $doorFactor)
                                                <li>{{ $doorFactor->doorStyle->product_label }} - {{ $doorFactor->factor }}</li>
                                            @endforeach
                                        </ul>
                                    @else
                                        <span class="badge bg-warning">No Factors</span>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
