    ...
    <script>
        $(document).ready(function() {
            ...
            $("#add_room_btn").click(function() {
                ...
                let uniqueRoomId = "room_" + new Date().getTime();

                let roomRow = `
                    <tr class="show_room table-warning" data-room-id="${uniqueRoomId}">
            ...
            $(document).on("click", "#stock_check_btn", function(event) {
                ...
                Swal.fire({
                    title: "Do you want shipping for stock check?",
                    showDenyButton: true,
                    confirmButtonText: "Yes",
                    denyButtonText: "No",
                }).then((result) => {
                    let shippingStatus = result.isConfirmed ? "yes" : "pending";

                    let roomsData = {};
                    $(".room_input").each(function(index) {
                        let roomRow = $(this).closest("tr");
                        let roomId = roomRow.attr("data-room-id"); // Now it should always be defined
                        let roomName = $(this).val().trim();
                    ...
                    $("#room_table_body tr[data-id]").each(function() {
                        let roomRow = $(this).prevAll(".show_room").first();
                        let roomId = roomRow.attr("data-room-id"); // Get the room ID from its row
                        let productId = $(this).data("id");
                        let quantity = parseInt($(this).find(".product_quantity").text()) ||
                            0;

                        let singleChecked = $(this).find(".single_check").prop("checked");
                        let doubleChecked = $(this).find(".double_check").prop("checked");

                        let checkboxStatus = "none";
                        if (singleChecked) checkboxStatus = "single";
                        if (doubleChecked) checkboxStatus = "double";

                        if (roomsData[roomId]) {
                            roomsData[roomId].products.push({
                                product_id: productId,
                                quantity: quantity,
                    ...
                    let comment = $("textarea[name='order_comment']").val();
                    console.log("Final Data Before Save:", roomsData); // Debugging in console

                    $.post("{{ route('stock_check.store') }}", {
                        _token: "{{ csrf_token() }}",
                        job_name: jobName,
                        rooms: roomsData,
                        assemble_cabinets_check: assembleRequired,
                        ...
                        Swal.fire({
                            ...
                            window.location.href = "{{ route('tenant_stock_check_index') }}";
                            // if (shippingStatus === "yes") {
                        ...
                        Swal.fire({
                            icon: "error",
                            title: "Error",
                            text: "An error occurred while saving the stock check.",
                            confirmButtonText: "OK",
                            confirmButtonColor: "#d33"
                        });
                    });
